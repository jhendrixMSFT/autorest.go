...
import (
	"armmultisharedmodelsgroup/storage"
	"context"
	"errors"
	"fmt"
...
// StorageAccountsServer is a fake server for instances of the storage.StorageAccountsClient type.
type StorageAccountsServer struct {
	...
	BeginCreateOrUpdate func(ctx context.Context, resourceGroupName string, accountName string, resource storage.StorageAccount, options *storage.StorageAccountsClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[storage.StorageAccountsClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method StorageAccountsClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, accountName string, options *storage.StorageAccountsClientGetOptions) (resp azfake.Responder[storage.StorageAccountsClientGetResponse], errResp azfake.ErrorResponder)
}
...
// The returned StorageAccountsServerTransport instance is connected to an instance of storage.StorageAccountsClient via the
...
func NewStorageAccountsServerTransport(srv *StorageAccountsServer) *StorageAccountsServerTransport {
	return &StorageAccountsServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[storage.StorageAccountsClientCreateOrUpdateResponse]](),
	}
}

// StorageAccountsServerTransport connects instances of storage.StorageAccountsClient to instances of StorageAccountsServer.
...
type StorageAccountsServerTransport struct {
	...
	beginCreateOrUpdate *tracker[azfake.PollerResponder[storage.StorageAccountsClientCreateOrUpdateResponse]]
}
...
func (s *StorageAccountsServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[storage.StorageAccount](req)
		if err != nil {
			return nil, err
		}
