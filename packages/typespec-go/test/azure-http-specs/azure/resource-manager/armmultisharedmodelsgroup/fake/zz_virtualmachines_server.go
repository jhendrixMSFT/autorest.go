...
import (
	"armmultisharedmodelsgroup/compute"
	"context"
	"errors"
	"fmt"
...
// VirtualMachinesServer is a fake server for instances of the compute.VirtualMachinesClient type.
type VirtualMachinesServer struct {
	...
	BeginCreateOrUpdate func(ctx context.Context, resourceGroupName string, vmName string, resource compute.VirtualMachine, options *compute.VirtualMachinesClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[compute.VirtualMachinesClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method VirtualMachinesClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, vmName string, options *compute.VirtualMachinesClientGetOptions) (resp azfake.Responder[compute.VirtualMachinesClientGetResponse], errResp azfake.ErrorResponder)
}
...
// The returned VirtualMachinesServerTransport instance is connected to an instance of compute.VirtualMachinesClient via the
...
func NewVirtualMachinesServerTransport(srv *VirtualMachinesServer) *VirtualMachinesServerTransport {
	return &VirtualMachinesServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[compute.VirtualMachinesClientCreateOrUpdateResponse]](),
	}
}

// VirtualMachinesServerTransport connects instances of compute.VirtualMachinesClient to instances of VirtualMachinesServer.
...
type VirtualMachinesServerTransport struct {
	...
	beginCreateOrUpdate *tracker[azfake.PollerResponder[compute.VirtualMachinesClientCreateOrUpdateResponse]]
}
...
func (v *VirtualMachinesServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		const regexStr = `/subscriptions/(?P<subscriptionId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/resourceGroups/(?P<resourceGroupName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Compute/virtualMachinesShared/(?P<vmName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 4 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		}
		body, err := server.UnmarshalRequestAsJSON[compute.VirtualMachine](req)
		if err != nil {
			return nil, err
		}
...
func (v *VirtualMachinesServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := v.srv.Get(req.Context(), resourceGroupNameParam, vmNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
