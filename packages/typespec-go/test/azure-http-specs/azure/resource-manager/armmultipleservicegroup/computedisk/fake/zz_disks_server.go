...
import (
	"armmultipleservicegroup/computedisk"
	"context"
	"errors"
	"fmt"
...
// DisksServer is a fake server for instances of the computedisk.DisksClient type.
type DisksServer struct {
	// BeginCreateOrUpdate is the fake for method DisksClient.BeginCreateOrUpdate
	// HTTP status codes to indicate success: http.StatusOK, http.StatusCreated
	BeginCreateOrUpdate func(ctx context.Context, resourceGroupName string, diskName string, resource computedisk.Disk, options *computedisk.DisksClientBeginCreateOrUpdateOptions) (resp azfake.PollerResponder[computedisk.DisksClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method DisksClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceGroupName string, diskName string, options *computedisk.DisksClientGetOptions) (resp azfake.Responder[computedisk.DisksClientGetResponse], errResp azfake.ErrorResponder)
}

// NewDisksServerTransport creates a new instance of DisksServerTransport with the provided implementation.
// The returned DisksServerTransport instance is connected to an instance of computedisk.DisksClient via the
// azcore.ClientOptions.Transporter field in the client's constructor parameters.
func NewDisksServerTransport(srv *DisksServer) *DisksServerTransport {
	return &DisksServerTransport{
		srv:                 srv,
		beginCreateOrUpdate: newTracker[azfake.PollerResponder[computedisk.DisksClientCreateOrUpdateResponse]](),
	}
}

// DisksServerTransport connects instances of computedisk.DisksClient to instances of DisksServer.
// Don't use this type directly, use NewDisksServerTransport instead.
type DisksServerTransport struct {
	srv                 *DisksServer
	beginCreateOrUpdate *tracker[azfake.PollerResponder[computedisk.DisksClientCreateOrUpdateResponse]]
}
...
func (d *DisksServerTransport) dispatchBeginCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	if beginCreateOrUpdate == nil {
		...
		body, err := server.UnmarshalRequestAsJSON[computedisk.Disk](req)
		if err != nil {
			return nil, err
		}
