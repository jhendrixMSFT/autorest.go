...
func tracerProvider(url string) (*tracesdk.TracerProvider, error) {
	...
	tp := tracesdk.NewTracerProvider(
		...
		tracesdk.WithResource(resource.NewWithAttributes(
			semconv.SchemaURL,
			semconv.ServiceNameKey.String("arraygroup"),
...
func newArrayClient(t *testing.T) (*ArrayClient, func(context.Context) error) {
	tp, err := tracerProvider("http://localhost:14268/api/traces")
	require.NoError(t, err)
	options := azcore.ClientOptions{}
	options.TracingProvider = azotel.NewTracingProvider(tp, nil)
	client, err := NewArrayClient(&options)
	require.NoError(t, err)
	return client, tp.Shutdown
}
...
func TestGetArrayEmpty(t *testing.T) {
	client, shutdown := newArrayClient(t)
